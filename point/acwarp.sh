...
note(){
echo "Related instructions:"
echo -e "1. To view the warp account status, please refer to this website: https://www.cloudflare.com/cdn-cgi/trace"
echo -e "	warp=on means ordinary free account,"
echo -e "	warp=plus means warp+ or teams team account,"
echo -e "	warp=off means non-warp network\n"
echo -e "2. This configuration has nothing to do with the current registered platform, and the IP region of the warp is determined according to the principle of the local network proximity of the actual platform used\n"
echo -e "3. The relevant parameters in this configuration can be applied to various platforms such as the Wireguard client, the Wireguard-warp outbound configuration of the Xray protocol, etc.\n"
echo -e "4. The IP port of the Endpoint in this configuration can be replaced with the optimized IP port of the actual platform’s local network test warp\n"
echo -e "5. In this configuration, PrivateKey, Address V6 address, reserved (optional), the three parameters are bound to each other\n"
...
clear
echo
echo "Warp account application successful" && sleep 2
echo
echo "warp-wireguard (wgcf) configuration information is as follows" && sleep 2
...
echo
echo "warp-wireguard (warp-go) configuration information is as follows" && sleep 2
cat warpgo.conf
echo
sleep 2
...
acwgcf(){
echo
echo "1.warp-wgcf common account"
echo "2.warp-wgcf+account"
echo "3.warp-wgcf+teams team account"
echo "0.exit"
...
acwarpgo(){
echo
echo "1.warp-go common account"
echo "2.warp-go+account"
echo "3.warp-go+teams team account"
echo "0.exit"
read -p "Please select: " menu
if [ "$menu" == "1" ];then
...
echo "Register to generate WARP-Wireguard configuration file, QR code"
echo "------------------------------------------------ ------"
echo
echo "1. Select warp-go to register warp configuration (support all account upgrades, display reserved value)"
echo "2. Select wgcf to register warp configuration (support all account upgrades)"
echo "3. Select warp api to register warp configuration (support display reserved value)"
echo "0. exit"
read -p "Please select: " menu
if [ "$menu" == "1" ];then
acwarpgo
